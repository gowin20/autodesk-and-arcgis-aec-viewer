// @ts-nocheck — vendored three.js r71 plugin code (constructor-function
// prototypes, dynamic properties); typing it would mean rewriting it.
/**
 * THREE.TransformControls + gizmo meshes — faithful port of
 * TransformGizmos.js from Philippe Leefsma's 2016 Forge extension
 * "Viewing.Extension.Transform" (library-javascript-viewer-extensions).
 *
 * The original was an IIFE mutating the global THREE; here it is an
 * installer invoked at runtime (SSR-safe: nothing runs at module scope).
 * LMV 7.* still bundles three.js r71, the era this code was written for.
 *
 * Integration changes vs the original (marked "MapLibre bridge"):
 *  - THREE.TransformControls takes an optional 4th `getRect` argument: the
 *    LMV canvas is display:none (zero rect), so screen-rect math must use
 *    impl.getCanvasBoundingClientRect(), which the bridge patches to the
 *    MapLibre canvas rect.
 *  - intersectObjects() builds its picking ray through the bridge-patched
 *    camera.viewportToRay (the camera is pinned: identity matrixWorld, the
 *    whole MapLibre view lives in projectionMatrix, and camera.isPerspective
 *    is false on LMV's UnifiedCamera, so both stock unproject paths are wrong).
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function installTransformGizmos(THREE: any): void {
	if (THREE.TransformControls) return; // already installed
  ...
  // MapLibre bridge: optional `getRect` supplies the visible canvas rect
  // (the LMV canvas is display:none, so its own bounding rect is useless).
  THREE.TransformControls = function ( camera, domElement, mode, getRect ) {
    ...
    this.update = function (highlight) {
      ...
      // MapLibre bridge: rect of the VISIBLE canvas, not the hidden LMV one.
      var rect = getRect ? getRect() : domElement.getBoundingClientRect();
    ...
    function intersectObjects( pointer, objects ) {
      ...
      // MapLibre bridge: rect of the visible canvas, and picking ray from
      // the bridge-patched camera.viewportToRay (pinned camera: identity
      // matrixWorld, full MapLibre view in projectionMatrix; isPerspective
      // is false on LMV's UnifiedCamera so both stock paths are wrong here).
      var rect = getRect ? getRect() : domElement.getBoundingClientRect();
      var x = ( ( pointer.clientX - rect.left ) / rect.width ) * 2 - 1;
      var y = - ( ( pointer.clientY - rect.top ) / rect.height ) * 2 + 1;

      if ( typeof camera.viewportToRay === 'function' ) {
        var viewportRay = camera.viewportToRay( new THREE.Vector2( x, y ) );
        ray.set( viewportRay.origin, viewportRay.direction );
